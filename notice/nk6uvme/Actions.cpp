...
void reset_action(Option* option, void *data) {
	Device * dev = (Device*) data;
	dev->reset();
	if (dev->getError() != SUCCESS) {
				cout << "Failed to reset the device.\n";
	}
}
...
void downloadFPGA_action(Option* option, void *data) {
	Device * dev = (Device*) data;
	fstream file;
	file.open(option->args[0], ios_base::out);
	if (file.is_open()) {
		size_t length;
		char * buffer = NULL;
		cout << "Downloading fpga firmware. Please wait." << endl;
		dev->downloadFPGAFirmware(&length, &buffer);
		if (dev->getError() == SUCCESS) {
			file.write(buffer, length);
			cout << "Firmware has been downloaded successfully.\n";
		} else
			cout << "Failed to download fpga firmware.\n";
	} else {
		cout << "Failed to open or create file: " << option->args[0] << endl;
	}
	file.close();
}
...
void devinfo_action(Option* option, void *data) {
	Device * dev = (Device*) data;
	char * version = dev->getFPGAVersion();
	if (dev->getError() != SUCCESS) {
...
void set_address_action(Option* option, void *data) {
	Device * dev = (Device*) data;
...
void set_version_action(Option* option, void *data) {
	Device * dev = (Device*) data;
	dev->setFPGAVersion(option->args[0]);
	if (dev->getError() != SUCCESS) {
...
void help_action(Option* option, void *data) {
	cout << "NOTICE Korea (c) 2012 <http://www.noticekorea.com/>\n"
			...
			"                             Option -dev has to be set.\n"
			"  -download-fpga <filename>  :Download fpga firmware to <filename> from \n"
			"                             the device.Option -dev has to be set.\n"
